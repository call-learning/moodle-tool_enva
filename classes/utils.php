...
class utils {
    ...
    /**
     * Count questions from categories, optionally only root questions.
     *
     * @param array $categories
     * @param bool $rootquestionly
     * @return int
     ...
     * @throws \dml_exception
     */
    public static function count_questions_from_categories(array $categories, bool $rootquestionly = true): int {
        global $DB;
        ...
        return $DB->count_records_sql($sql, $qcparams);
    }
     ...
     * @param string|null $onlyoftype The type of question to consider for purging.
     * @return array An array containing the status, question data, and usage count.
     */
    public static function purge_question(int $questionid, ?int $olderthan = null, ?string $onlyoftype = null): array {
        ...
        if ($onlyoftype !== null && $question->qtype !== $onlyoftype) {
            $returnval['status'] = 'wrongtype';
            return $returnval;
        }
        if ($usagecount == 0) {
